...
enum ButtonEnum {
        ...
        BTN_BLACK, // Available on the original Xbox controller
        BTN_WHITE, // Available on the original Xbox controller
        /**@}*/
...
inline constexpr int8_t ButtonIndex(ButtonEnum key) {
    ...
    return
        (key == UP) ? 0 : // || key == RED
        (key == RIGHT) ? 1 : // || key == YELLOW
        (key == DOWN) ? 2 : // || key == GREEN
        (key == LEFT) ? 3 : // || key == ORANGE
        (key == SELECT || key == SHARE || key == BACK || key == VIEW || key == CREATE || key == CAPTURE) ? 4 : // || key == BLUE 
        (key == START || key == OPTIONS || key == MENU || key == PLUS) ? 5 :
        (key == L3 || key == TWO) ? 6 :
        (key == R3 || key == ONE) ? 7 :
        (key == L2 || key == LT || key == MINUS || key == BTN_BLACK) ? 8 :
        (key == R2 || key == RT || key == HOME || key == BTN_WHITE) ? 9 :
