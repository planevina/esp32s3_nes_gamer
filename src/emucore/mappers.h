#define NES_CYCLES_PER_SCANLINE (nes_getcontextptr()->cycles_per_scanline)
//--------------------------------------------------------------------------------
//MAPPERS !!!
//................................................................................
//MAPPER0
...
/* Used when tile $FD/$FE is accessed */
void mmc10_latchfunc(uint32_t address, uint8_t value)
{
    int reg;
    ...
    mmc_bankvrom(4, address, regs10[reg]);
}

void map10_write(uint32_t address, uint8_t value)
{
    switch ((address & 0xF000) >> 12)
    {
    case 0xA:
...
void map10_getstate(SnssMapperBlock *state)
{
    state->extraData.mapper10.latch[0] = latch10[0];
    state->extraData.mapper10.latch[1] = latch10[1];
    state->extraData.mapper10.lastB000Write = regs10[0];
    state->extraData.mapper10.lastC000Write = regs10[1];
    state->extraData.mapper10.lastD000Write = regs10[2];
    state->extraData.mapper10.lastE000Write = regs10[3];
}

void map10_setstate(SnssMapperBlock *state)
{
    latch10[0] = state->extraData.mapper10.latch[0];
    latch10[1] = state->extraData.mapper10.latch[1];
    regs10[0] = state->extraData.mapper10.lastB000Write;
    regs10[1] = state->extraData.mapper10.lastC000Write;
    regs10[2] = state->extraData.mapper10.lastD000Write;
    regs10[3] = state->extraData.mapper10.lastE000Write;
}

 map_memwrite map10_memwrite[] =
{
...
void map10_init()
{
    regs10[0] = regs10[1] = regs10[2] = regs10[3] = 0x00;
    latch10[0] = latch10[1] = 0xFE;
...
uint8_t map19_read(uint32_t address)
{
    int reg = address >> 11;
    ...
    {
    case 0xA:
        return irq19.counter & 0xFF;

    case 0xB:
        return irq19.counter >> 8;
...
void map19_hblank(int scanline)
{
    if (irq19.enabled)
    {
        irq19.counter += 105;  //114改为了 105  ， 

        if (irq19.counter >= 0x7FFF)
        {
            nes6502_irq();
            irq19.counter = 0x7FFF;
            irq19.enabled = 0;
        }
    }
}
 ...
 void map73_hblank (int vblank)
{
  ...
  if (irq73.enabled)
  {
    /* Is there a constant for cycles per scanline? */
    /* If so, someone ought to substitute it here   */
    //这里这个114，隔壁是 #define NES_CYCLES_PER_SCANLINE (nes_getptr()->cycles_per_scanline)
