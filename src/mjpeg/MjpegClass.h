...
public:
    bool setup(
        Stream *input, uint8_t *mjpeg_buf, JPEG_DRAW_CALLBACK *pfnDraw,JPEG_DRAW_CALLBACK *pfnDraw2, bool useBigEndian,
    ...
    //这里我修改了，变成了两个回调函数，根据单双帧选择不同的回调
    //buffer_id 两个值，1 为单帧，0为双帧（非1都可以）
    bool decodeJpg(uint8_t buffer_id)
    {
        _remain = _mjpeg_buf_offset;
        _jpeg.openRAM(_mjpeg_buf, _remain, buffer_id ? _pfnDraw : _pfnDraw2);
        if (_scale == -1)
        {
            // scale to fit width
            int iMaxMCUs;
            ...
            float ratio = (float)w / _widthLimit;
            if (ratio <= 1)
            {
                _scale = 0;
